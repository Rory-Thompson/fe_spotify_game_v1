//logic is to be kept seperate here from the stuff that is actually calling these functions to interact with the dom so it can be used for testing.
...
class Question {
    ...
    constructor(data) {
        ...
        console.log("this is the data parsed into Question base constructor: ", data);
        this.id = data.id;
...
async function get_artist_image(artist_code) {
    if ((artist_code == null) || myTokens == null) {
        return null
    };
    const response = await fetch(`https://api.spotify.com/v1/artists/${artist_code}`, {
        headers: {
    ...
    console.log("these are my playlists: ", artist);
    return [artist.images[artist.images.length-1], artist.name]
}